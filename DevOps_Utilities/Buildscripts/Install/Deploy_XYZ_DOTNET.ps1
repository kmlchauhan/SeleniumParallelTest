    ...
    function CreateAppPoolAndWebSite([string]$protocolName,[string]$ServiceName,$networkCred,[string]$server,$credential)
    {
		  ...
		  if (!(Test-Path $ReqWebSiteName -pathType container))
			...
			Write-Host ==> "IIS:\Sites\"$ReqWebSite -physicalPath "$PhysicalDirectoryPath" -bindings @{protocol="http";bindingInformation=":8080:"} ==>
			New-Item "IIS:\Sites\$ReqWebSite" -physicalPath "$PhysicalDirectoryPath" -bindings @{protocol="http";bindingInformation=":8080:"}
			}
	...
	$xml = [xml](get-content $root\Deploy_MZ00_DOTNET.xml)
	 ...
	 #===================================================================================== 
	  ...
	  $tmpDomain=$xml.CMSWebDeployment.AppPoolIdentityCredentials.Domain
	  $Domain=$tmpDomain
	  $Username=$xml.CMSWebDeployment.AppPoolIdentityCredentials.Username
	  $Password=$xml.CMSWebDeployment.AppPoolIdentityCredentials.Password
	  
	  $iisreset = $xml.CMSWebDeployment.IISRESET
	 ...
	 #===================================================================================== 
	  write-host
		...
		$services = $xml.CMSWebDeployment.ServiceToInstall
	  ...
	  Try
	  {
		...
		foreach ($serviceAttribute in $services.ChildNodes) 
		{
			...
			write-host $PhysicalDirectoryPath -foreground "DarkGray" -backgroundcolor "White"
			write-host " ==> Service Name = "$service " *********** AppPoolStatus = "$AppPoolStatus   "********* Deployment Flag = " $DeploymentFlag  -foreground "DarkGray" -backgroundcolor "White"
